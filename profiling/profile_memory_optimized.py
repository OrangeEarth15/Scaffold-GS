"""
Scaffold-GS inference VRAM profiling - OPTIMIZED VERSION (完整优化)

包含全部优化 (G1-G4 PyTorch + R1-R3 CUDA):

PyTorch Generate 优化:
- G1: Early Opacity Pruning - opacity mask 后立即过滤
- G2: FP16 Mixed Precision - MLP 推理使用 FP16
- G3: Conditional Concatenation - 按需拼接 distance 特征
- G4: In-place Operations - 减少临时 tensor

CUDA Rasterize 优化 (需重新编译):
- R1: BinningState In-place Sorting - 排序复用 buffer
- R2: ImageState Ranges 修复 + uint16_t - 修正过度分配
- R3: Inference Mode - 条件分配 backward-only buffers

实测效果 (MipNeRF360 Garden, 2594×1680):
- Generate 峰值: 670.8 MB (baseline 1,190.7 MB, -43.7%)
- Rasterize 峰值: 894.4 MB (baseline 1,117.0 MB, -19.9%)

Usage:
  python profiling/profile_memory_optimized.py -m <model_path>
"""
import os
import sys
import torch
...
def profile_one_frame(view, gaussians, pipeline, background):
    ...
    # ========== Optimization: Conditional concatenation ==========
    ...
    if need_dist:
        cat_local_view = torch.cat([cat_local_view_wodist, ob_dist], dim=1)  # [V, 36]
    else:
        cat_local_view = None  # Save memory
    # 可选: appearance embedding (per-camera)
    if pc.appearance_dim > 0:
        camera_indicies = torch.ones_like(cat_local_view_wodist[:,0], dtype=torch.long, device=ob_dist.device) * view.uid
        appearance = pc.get_appearance(camera_indicies)
    torch.cuda.synchronize()
    log_mem("2c. view features + cat")

    # ========== FP16 Mixed Precision + Early Opacity Pruning ==========
    # 优化2: 使用 autocast 让 MLP 计算在 FP16 上进行，利用 Tensor Core 加速
    # 优化1: 在 reshape 后立即应用 mask 过滤，避免分配完整的 [G, 22] 张量
    with torch.amp.autocast('cuda', enabled=True):
        ...
        torch.cuda.synchronize()
        log_mem("2d. MLP opacity + mask (FP16)")
        # 打印存活比例，用于验证 opacity_survive_ratio 假设
        print(f"       generated: {n_generated:,}, survived: {n_survived:,} ({n_survived/n_generated*100:.1f}%)")

        # ── Step 2e: MLP 推理 color (优化：立即过滤) ──
        # ========== Early Opacity Pruning Optimization ==========
        # MLP 输入仍然是 [V, dim]，输出 [V, 3*n_offsets]，reshape 后立即过滤
        if pc.appearance_dim > 0:
        ...
        color = color.reshape([anchor.shape[0] * pc.n_offsets, 3])[mask]  # [G,3]->[S,3] 立即过滤
        torch.cuda.synchronize()
        log_mem("2e. MLP color (FP16 + early filter)")

        # ── Step 2f: MLP 推理 cov (优化：立即过滤) ──
        if pc.add_cov_dist:
            scale_rot = pc.get_cov_mlp(cat_local_view)
        else:
            scale_rot = pc.get_cov_mlp(cat_local_view_wodist)
        scale_rot = scale_rot.reshape([anchor.shape[0] * pc.n_offsets, 7])[mask]  # [G,7]->[S,7] 立即过滤
        torch.cuda.synchronize()
        log_mem("2f. MLP cov (FP16 + early filter)")

    # 转回 FP32 供 rasterizer 使用（要求 Float 输入）
    opacity = opacity.float()
    color = color.float()
    scale_rot = scale_rot.float()
    torch.cuda.synchronize()
    log_mem("2f+. convert FP16 -> FP32")

    # ── Step 2g: offsets 过滤 ──
    offsets = grid_offsets.view([-1, 3])[mask]  # [G, 3] -> [S, 3]
    torch.cuda.synchronize()
    log_mem("2g. filter offsets")

    # ── Step 2h: Use indexing instead of repeat to avoid memory copy ──
    # ========== Optimization: indexing instead of repeat ==========
    ...
    torch.cuda.synchronize()
    log_mem("2h. indexing (no repeat!) ★ GEN PEAK")

    # ── Step 2i: post-process with in-place ops ──
    ...
    offsets.mul_(scaling_repeat[:,:3])  # in-place
    xyz = repeat_anchor.add(offsets)  # avoid creating temp with +
    torch.cuda.synchronize()
    log_mem("2i. post-process (xyz/color/opacity/scaling/rot)")
    ...
    if cat_local_view is not None:
        del cat_local_view
    del cat_local_view_wodist, neural_opacity
    del anchor_indices, scaling_repeat, repeat_anchor
    if pc.appearance_dim > 0:
        del appearance, camera_indicies
    torch.cuda.synchronize()
    # 此时 alloc 应骤降 (中间张量已释放), 但 reserved 可能仍高 (caching pool 未归还)
    log_mem("2j. after del intermediates (optimized)")

    # 强制把 caching pool 的空闲块归还给 CUDA
    # 这样 Phase 3 从干净基线开始
    torch.cuda.empty_cache()
    torch.cuda.synchronize()
    log_mem("2k. after empty_cache")
